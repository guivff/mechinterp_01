...
- **N3 as a norm-matched null.** Its ‖ΔW‖_F is 2.069, 24 % above A's 1.675; it is a reference. **N3 scaled to the arm's ‖ΔW‖_F is the comparable null** (limitation 7): A 5.7×, A s1 4.2×, C_masked 2.2×, B 2.6× above it. A's V seed ratio is 1.363 (0.1252 / 0.0919; 1.362 from the rounded values).
...
7. **New (F10, 2026-09-05):** "The 'floor' beside each trace is the split-half statistic of that arm's own difference vector (12–15 % of the trace for every trained arm; B 18 %, N3 29 %) and measures snippet-half stability, not detectability. The nulls that answer 'is there a trace?' are the base-vs-base split-half N1 = 0.747 at neutral p1 — an unpaired statistic that exceeds A's paired trace and is not its noise floor — and the untrained adapter N3 scaled to the arm's ‖ΔW‖_F, which is the comparable null: A's trace is 5.7× its scaled N3 (0.210 vs 0.037; A s1 4.2×), C_masked's 2.2× (0.286 vs 0.129), B's 2.6×."
