...
| P4 / H3 (preregistered) | Shuffled-reward GRPO (B) shares A's direction at lower magnitude | **refuted** — A·B = −0.13 on neutral text, below all 50 random draws (anti-aligned); passes on math only |
...
C reaches A's held-out accuracy — 0.930 vs 0.940, McNemar 7/5, p = 0.77, **identical under both parsers** (no cut fires on any A or C completion). Its neutral-text trace at L15 p1 is **3.49 vs A's 0.21 (seed 0) / 0.155 (seed 1): 16.6–22.6× larger**. The gap decomposes as ~4.2× larger ‖ΔW‖_F (6.96 vs 1.68) × ~4.0× larger trace-per-unit-weight-change (V = 0.50 vs 0.125). C is half-aligned with A (cos +0.505, above all 50 random draws); B is anti-aligned (−0.13). Same data, same behaviour, different learning rule — the difference is the weighting.

Caveats carried with the claim: C is single-seed and fixed-budget (12 % of its corpus), unmasked, inherits A's formatting, and C·D_math_full = 0.55 leaves an SFT/corpus reading open. The claim is narrow and hedged: one 4B base model, one task.
...
- Steering at α ≤ 0.5 clears the five-seed random null (A: 0.200 vs 0.110–0.170, p = 0.013; D_math_full 0.285) but accuracy and EOS rate rise together, so those gains are **not separated from the same stopping effect**. Steering and Gate 2 are not independent support.
...
It is a narrow, hedged result on one model and one task with the imitation arm at n = 1 (possibly 2): imitation SFT at matched accuracy leaves a 16.6–22.6× larger trace than the GRPO run it imitates, so the small RL trace is a property of the learning rule. It is also a worked example of a metric artifact caught before write-up by reading the data, with three independent methods converging on the same items. It is not evidence that "RL leaves no trace," not a replication of Minder, not a causal claim about steering, and not a test of the SGD-specific theory.
