...
| C_masked (completion-only C; results/acc_C_masked_s0.json) | 187/200 = 0.935 | 187/200 = 0.935 | 0 | 0 |
...
| C_masked vs A | 4 / 5, p = 1.00 | 4 / 5, p = 1.00 (identical) |
| C_masked vs C seed 0 | 5 / 4, p = 1.00 | 5 / 4, p = 1.00 (identical) |
| C_masked vs D_math | 60 / 5, p < 0.001 | 21 / 7, p = 0.013 |
| A vs D | 136 / 1 | — |
...
| C_masked (results/perposition_table_C_masked.csv) | 0.286 (0.039) / 0.257 (0.042) | 0.645 (0.014) / 0.651 (0.086) | 0.252 / 0.189 | 2.26 / 2.53 |
...
| **C_masked** (completion-only C, seed 0; results/lora_delta_stats_C_masked.json, visibility_table_C_masked.md) | 0.286 | 5.844 | 0.551 | 0.3550 | **0.0489** | 0.1103 |
...
Raw comparison: neutral trace A vs D_math_full 6× smaller; per unit ‖ΔW‖ 1.4× smaller; vs D 3× smaller. **C vs A, four seed pairs (C s0/s1 × A s0/s1): V **4.0–5.5× (four (C, A) seed pairs: 4.00, 5.45, 4.01, 5.47)** per unit ‖ΔW‖_F was the learning-rule candidate (resolved below); the raw ratios 16.63× / 22.57× / 16.68× / 22.63× (`results/trace_ratio_C_A_seeds.csv`) are descriptive. **Dose is not matched and this is the primary open confound.** C was trained at lr 1e-4 × 225 SFT steps (lr×steps = 2.25e-2); A at lr 3e-5 × 150 GRPO steps (4.5e-3) — a **5.0× lr×steps mismatch**. The raw trace ratio factorises exactly as ‖ΔW‖_F × V: **16.63× = 4.157× (‖ΔW‖) × 4.001× (V)** against A seed 0, and **22.57× = 4.139× × 5.453×** against A seed 1 (C seed 1: 16.68× = 4.154 × 4.015; 22.63× = 4.137 × 5.470). The ~4.2× weight-change factor is what a 5× lr×steps mismatch would predict; the raw 16.63–22.63× is descriptive only. A dose-matched C family was not run. **Loss placement (§12b): V is 0.059 for masked math SFT, below both A seeds, and 0.179 for the same corpus unmasked at unchanged ‖ΔW‖_F; C was trained unmasked while GRPO's loss touches only completion tokens.** **Resolved 2026-09-05 03:05 by C_masked (§9, §12b; `results/REPLICATION_REPORT_C_masked.md`): V(C_masked) = 0.049 ≤ 0.18 → loss placement explains most of the C-vs-A gap.** The primary reading is loss placement: prompt-token supervision gives a large readable trace, completion-only supervision (GRPO, masked SFT) a small one, at matched accuracy. The learning-rule reading is **not supported by the decisive test**. The dose confound is **excluded as the explanation** — C_masked has C's dose (lr 1e-4 × 225 steps, ‖ΔW‖_F 5.84, 84 % of C's) and its trace is 0.286, i.e. **1.36–1.85× A's (0.286/0.210, 0.286/0.155; ≈1.4–1.9×)** against 16.63–22.63× for unmasked C. One C_masked seed; hedged. **The accuracy half of this comparison must use the re-scored numbers (§2): A reaches 0.94 where the base is already 0.79, not 0.14, so "A achieved +0.80 accuracy with a 4× smaller ‖ΔW‖" is not a sayable claim.**
...
- Geometry L15 (raw ‖d‖ / floor / constancy): neutral p1 **3.488** / 0.435 / 0.274, p2 2.434 / 0.423 / 0.171; math p1 5.380 / 0.152 / 0.674, p2 5.251 / 0.402 / 0.468; math/neutral 1.54 / 2.16. **At matched accuracy, C's per-unit-‖ΔW‖ trace V is 4.0–5.5× (four (C, A) seed pairs: 4.00, 5.45, 4.01, 5.47) A's — the learning-rule candidate until C_masked (bullet below) resolved it in favour of loss placement. The raw norm ratio is 16.63× (3.488 vs 0.210) and 22.57× (vs 0.155), descriptive only.** **Dose is not matched and this is the primary open confound.** C was trained at lr 1e-4 × 225 SFT steps (lr×steps = 2.25e-2); A at lr 3e-5 × 150 GRPO steps (4.5e-3) — a **5.0× lr×steps mismatch**. The raw trace ratio factorises exactly as ‖ΔW‖_F × V: **16.63× = 4.157× (‖ΔW‖) × 4.001× (V)** against A seed 0, and **22.57× = 4.139× × 5.453×** against A seed 1. The ~4.2× weight-change factor is what a 5× lr×steps mismatch would predict; the raw 16.63–22.63× is descriptive only. A dose-matched C family was not run; **C_masked (below) has C's dose and A's loss placement, and lands at A's trace — so dose is excluded as the explanation of the gap and the learning rule is not supported by the decisive test.** At position 2 the raw ratio is 13.21× (2.434 vs 0.184). **With C seed 1 the neutral-p1 ratio has a range on both sides — four seed pairs, C s0/A s0 16.63×, C s0/A s1 22.57×, C s1/A s0 16.68×, C s1/A s1 22.63× (`results/trace_ratio_C_A_seeds.csv`); p2 13.21× / 16.52× / 13.48× / 16.85×.** The spread is entirely A's: C's two seeds differ by 0.3 % in norm, A's by 26 %.
...
- **C_masked — the decisive loss-placement test (R1, separate pod, $2.01; `results/REPLICATION_REPORT_C_masked.md`).** Same corpus (`data/C_samples.jsonl` sha 78022b70…), same recipe as C (lr 1e-4, 225 steps, batch 8, r 32/α 64, seed 0), **loss on completion tokens + EOS only** — the mechanism D_math used and the placement GRPO has; supervised fraction of selected tokens **0.726** (`results/supervised_fraction_C_masked.json`). Held-out **187/200 = 0.935 under both parsers** (no cut fires); vs A 4/5 p = 1.00, vs C s0 5/4 p = 1.00, vs D_math 60/5 raw → 21/7 re-scored p = 0.013. Geometry L15 (raw ‖d‖ / floor / constancy): neutral p1 **0.286** / 0.039 / 0.252, p2 0.257 / 0.042 / 0.189; math p1 0.645 / 0.014 / 0.766, p2 0.651 / 0.086 / 0.310; math/neutral 2.26 / 2.53 (A: 2.30 / 2.78). **‖ΔW‖_F = 5.844** (84 % of C's 6.963), max module 0.551 (`layers.2.mlp.up_proj`), top σ 0.3550 → **V(neutral) = 0.049, below both A seeds (0.125 / 0.092) and below masked D_math (0.059)**; the 12× drop in the neutral trace (3.49 → 0.29) at a 16 % drop in ‖ΔW‖ is not a weight-norm effect. Trace ratio to A at neutral p1 **1.36 / 1.85** (`results/perposition_table_C_masked_ratio.csv`). Cosines p1 / p2 (`results/perposition_table_C_masked_cosine.csv`): **C_masked·A s0 0.624 / 0.584 (neutral)**, 0.570 / 0.743 (math); ·A s1 0.494 / 0.436, 0.521 / 0.726; ·C s0 0.320 / 0.268, 0.362 / 0.540; ·C s1 0.297 / 0.252; ·D_math 0.187; ·D_math_full 0.230; ·N1 halves −0.068. C_masked is closer in direction to A (0.62, about A's own cross-seed 0.68) than to unmasked C (0.32). Patchscope λ=1 p1: neutral `→ 1 0 \n 2 9 8 - 5 3 6 7 … 4 Let -> > now I lets`; math function words and symbols (`high < we { true our …`). **Decision line (pre-stated, §12b): V = 0.049 ≤ 0.18 → loss placement explains most of the C-vs-A gap.** Not run: seed 1, other layers/positions, emergence, steering, relevance grading.
...
- **`figs/fig1_norm_vs_accuracy.png` — headline.** One panel: trace norm ‖d̄‖ at L15 position 1 on neutral text (log y) against re-scored held-out accuracy (x), every arm. C, **C_masked** and A dark (C_masked added 2026-09-05 as the third dark bar between C and A, from `results/perposition_table_C_masked.csv` and `results/acc_C_masked_s0.json`), other arms low opacity; each bar runs from that arm's paired split-half floor up to its measured norm; N3 is a horizontal floor line; base is a vertical line (accuracy 0.790, no trace by construction). The brackets mark **16.63–22.63×** (C vs A, four seed pairs, `results/trace_ratio_C_A_seeds.csv`; C seed 0 plotted, seed 1 differs by 0.3 % in norm) and **1.36–1.85×** (C_masked vs A, V 0.049). Sources: `results/perposition_table_C.csv`, `results/perposition_table_seeds.csv`, `results/perposition_table_A_seeds.csv`, `results/acc_table_reparsed.md`.
