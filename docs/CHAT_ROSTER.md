...
| **C2 — blind tagger** | fresh chat, **other account** | ChatGPT or Claude | 68 tag rows + tallies | `V2_TAGGER_LLM_JUDGE.md`, `discordant_A_vs_D_math_readable.md` | `discordant_key.json`, `discordant_sample20.txt`, the unblinded 20-item md, any project doc, the hypothesis | nothing; send now |
...
| **RT — results red-team** | fresh chat, other account | — | ranked top-5 objections with answered/partial/unanswered | current digest + firewall **only** | the write-up (objections must target evidence, not prose) | nothing; send now |
