...
1. **Headline (C vs A).** Imitation SFT on A's own correct samples reaches A's accuracy (0.930 vs 0.940, p = 0.77, identical under both parsers) and leaves **4.0× (A s0) to 5.4× (A s1) more trace per unit ‖ΔW‖_F**. The raw neutral-text norm ratio, **16.6–22.6×** (3.49 vs 0.21 / 0.155), is descriptive only. **The learning-rule claim is the per-unit-‖ΔW‖ factor V: 4.001× (A s0) / 5.453× (A s1).** The raw ratio factorises exactly: 16.63× = 4.157 × 4.001 and 22.57× = 4.139 × 5.453. C was trained at lr 1e-4 × 225 SFT steps (lr×steps 2.25e-2) vs A's 3e-5 × 150 GRPO steps (4.5e-3) — a **5.0× lr×steps mismatch** that plausibly accounts for the ‖ΔW‖ factor; no dose-matched C family was run. **Named as the primary open confound in the same sentence as the claim.** C single seed. C·A = +0.505, above all 50 random draws; B·A = −0.13, below all 50 (orthogonal to slightly negative). "The difference is the weighting" is deleted everywhere.
...
5. **Steering does not survive the parser correction — changed 2026-09-05.** The raw-parser grid clears a five-seed random null at α ≤ 0.5 (A 0.200, p = 0.013; C 0.215; D_math_full 0.285). Re-scored on the same completions (`results/steer_table_reparsed.md`): unsteered **0.790**, A α=0.25 **0.815 (p = 0.52)**, C α=0.5 **0.730**, D_math_full α=0.5 **0.650 (p = 0.0003 the wrong way)**, random 0.765–0.820. **No direction beats the unsteered base; the raw gains were the steering making the model stop.** Steering is not causal support and should be one hedged sentence or dropped. Guiv's reading of the α = 1 generations: ⏳.
