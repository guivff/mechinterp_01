...
One row per headline number in the current digest (rows 46–50 added 2026-09-05 at the `replication` merge (C seed 1, V-range string); rows 53–60 at the C_masked merge (1d33df3; 51–52 reserved for Guiv's readings; 58 = the corrected decomposition; 59–60 = C_masked ‖ΔW‖ and C_masked:A ratio, renumbered from 51–52 in C2)). **The agent filled only the first four columns.** The last three are Guiv's and are deliberately empty:
...
| 51 | (reserved) Guiv's reading — cooking corpus, `notes/READ_cooking_corpus.md` | not in this worktree; Guiv fills | `results/review_packet/cooking_samples.md` | Guiv |  |  |  |
| 52 | (reserved) Guiv's reading — black-box rows, `notes/READ_blackbox_rows.md` | not in this worktree; Guiv fills | `results/review_packet/blackbox_rows.md` | Guiv |  |  |  |
| 53 | **C_masked V(neutral p1) — the decision-line number** | 0.286 / 5.844 = 0.049 ≤ 0.18 → loss placement (line pre-stated in digest §12b / T2 §1.1) — one-liner output: `V = 0.0489 -> loss placement (<=0.18)` | `results/visibility_table_C_masked.md`, `results/lora_delta_stats_C_masked.json`, `results/perposition_table_C_masked.csv` | Chat 3 R1 / tools/visibility_C_masked.py |  |  |  |
| 54 | C_masked trace, floor, constancy, L15 neutral p1 / p2 | 0.286 / 0.039 / 0.252 ; 0.257 / 0.042 / 0.189 (math p1 0.645 / 0.014 / 0.766) — one-liner output: `neutral p1 0.286 0.039 0.252 / neutral p2 0.257 0.042 0.189 / math p1 0.645 0.014 0.766 / math p2 0.651 0.086 0.31` | `results/perposition_table_C_masked.csv` | Chat 3 R1 / tools/per_position_diff.py |  |  |  |
| 55 | C_masked held-out accuracy, both parsers | 187/200 = 0.935 raw and re-scored (no cut fires); vs A s0 4/5 p = 1.00; vs C s0 5/4 p = 1.00 — one-liner output: `raw 187 / 200 | re-scored 187 | cuts fired 0; acc_A_s0 C_masked-only 4 other-only 5 p=1.000; acc_C_s0 C_masked-only 5 other-only 4 p=1.000` (vs C s1: 7 / 5, p = 0.774) | `results/acc_C_masked_s0.json`, `results/acc_table_C_masked.md` | Chat 3 R1 / grpo/eval_acc.py |  |  |  |
| 56 | C_masked cosines, L15 neutral p1 / p2 | ·A s0 0.624 / 0.584; ·A s1 0.494 / 0.436; ·C s0 0.320 / 0.268; ·C s1 0.297 / 0.252 — one-liner output: `C_s0 p1 0.32, C_s1 p1 0.297, A_s0 p1 0.624, A_s1 p1 0.494, C_s0 p2 0.268, C_s1 p2 0.252, A_s0 p2 0.584, A_s1 p2 0.436` | `results/perposition_table_C_masked_cosine.csv` | Chat 3 R1 |  |  |  |
