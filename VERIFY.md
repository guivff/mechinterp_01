...
One row per headline number in the current digest (rows 46–50 added 2026-09-05 at the `replication` merge (C seed 1, V-range string); rows 53–60 at the C_masked merge (1d33df3; 58 = the corrected decomposition; 59–60 = C_masked ‖ΔW‖ and C_masked:A ratio); 51–52 = Guiv's readings and 61–64 = his discordant block, merged from `origin/guiv-notes` (b38d076); 65–72 = R2 / E2 / null table (C5)). **The agent filled only the first four columns.** The last three are Guiv's and are deliberately empty:
...
|---:|---|---|---|---|---|---|---|
| 1 | Held-out accuracy, base, raw last-number parser | 28/200 = 0.140 | `results/acc_base_s0.json` | pod runner / grpo/eval_acc.py | `tools/guiv_recompute.sh` row 1 → `notes/guiv_recompute_output.txt`; matches |  |  |
| 2 | Held-out accuracy, base, stopping-robust re-parse | 158/200 = 0.790 | `results/acc_table_reparsed.md` | pod runner / tools/reparse_acc.py | `tools/guiv_recompute.sh` row 2 → `notes/guiv_recompute_output.txt`; matches |  |  |
| 3 | Held-out accuracy, A, both parsers (unchanged) | 188/200 = 0.940 | `results/acc_A_s0.json, results/acc_table_reparsed.md` | pod runner / grpo/eval_acc.py | not independently recomputed by me (Guiv) |  |  |
| 4 | Held-out accuracy, C, both parsers (unchanged) | 186/200 = 0.930 | `results/acc_C_s0.json, results/acc_table_reparsed.md` | pod runner | `tools/guiv_recompute.sh` row 4 → `notes/guiv_recompute_output.txt`; matches |  |  |
| 5 | Held-out accuracy, B, raw | 15/200 = 0.075 | `results/acc_B_s0.json` | pod runner | not independently recomputed by me (Guiv) |  |  |
| 6 | Held-out accuracy, B, re-parsed | 162/200 = 0.810 | `results/acc_table_reparsed.md` | pod runner | not independently recomputed by me (Guiv) |  |  |
| 7 | Held-out accuracy, D_math, raw / re-parsed | 132/200 = 0.660 / 173/200 = 0.865 | `results/acc_D_math_s0.json, results/acc_table_reparsed.md` | pod runner | not independently recomputed by me (Guiv) |  |  |
| 8 | Held-out accuracy, D_math_full, raw / re-parsed | 127/200 = 0.635 / 164/200 = 0.820 | `results/acc_D_math_full_s0.json, results/acc_table_reparsed.md` | pod runner | not independently recomputed by me (Guiv) |  |  |
| 9 | Held-out accuracy, D, raw / re-parsed | 53/200 = 0.265 / 108/200 = 0.540 | `results/acc_D_s0.json, results/acc_table_reparsed.md` | pod runner | not independently recomputed by me (Guiv) |  |  |
| 10 | A vs base paired, raw | 162 / 2 discordant, McNemar p < 1e-6 | `results/acc_table.md` | pod runner / tools/acc_table.py | not independently recomputed by me (Guiv) |  |  |
| 11 | A vs base paired, re-parsed | 35 / 5 discordant, p = 1e-6 | `results/acc_table_reparsed.md` | pod runner / tools/reparse_acc.py | not independently recomputed by me (Guiv) |  |  |
| 12 | A vs D_math paired, raw / re-parsed | 62 / 6 → 22 / 7, p<1e-6 → p=0.0081 | `results/acc_table_reparsed.md` | pod runner | `tools/guiv_recompute.sh` row 12 → `notes/guiv_recompute_output.txt`; matches |  |  |
| 13 | A vs C paired (identical under both parsers) | 7 / 5, p = 0.774 | `results/acc_table.md, results/acc_table_reparsed.md` | pod runner | not independently recomputed by me (Guiv) |  |  |
| 14 | Of the 62 raw A-only-correct items, how many survive | 22 survive; 40 become both-correct; A loses 0 | `results/acc_table_reparsed.md, results/reparse_rescued_ids.json` | pod runner | not independently recomputed by me (Guiv) |  |  |
| 15 | Re-parser audit: rescues that are genuine | 20/20 sampled, 0 false rescues (6.2% coverage of 322) | `results/reparse_audit.md` | pod runner / tools/reparse_audit.py; agent's reading | not independently recomputed by me (Guiv) |  |  |
| 16 | D_math items rescued by the re-parser | 41 (0 broken) | `results/reparse_rescued_ids.json` | pod runner | not independently recomputed by me (Guiv) |  |  |
| 17 | Per-position ‖d‖, D, L15 neutral p1 (floor) | 3.151 (0.400) | `results/perposition_table_C.csv` | pod runner / tools/perposition_table.py | `tools/guiv_recompute.sh` row 17 → `notes/guiv_recompute_output.txt`; matches |  |  |
| 18 | Per-position ‖d‖, C, L15 neutral p1 (floor) | 3.488 (0.435) | `results/perposition_table_C.csv` | pod runner | not independently recomputed by me (Guiv) |  |  |
| 19 | Per-position ‖d‖, A, L15 neutral p1 (floor) | 0.210 (0.029) | `results/perposition_table_C.csv` | pod runner | not independently recomputed by me (Guiv) |  |  |
| 20 | Per-position ‖d‖, B, L15 neutral p1 (floor) | 0.094 (0.017) | `results/perposition_table_C.csv` | pod runner | not independently recomputed by me (Guiv) |  |  |
| 21 | Per-position ‖d‖, N3 untrained floor, L15 neutral p1 | 0.046 (0.013) | `results/perposition_table_C.csv` | pod runner | not independently recomputed by me (Guiv) |  |  |
| 22 | Headline trace ratio C : A on neutral p1 | 3.488 / 0.210 = 16.6× (digest §9 states 17×) | `results/perposition_table_C.csv` | pod runner | `tools/guiv_recompute.sh` row 22 → `notes/guiv_recompute_output.txt`; matches |  |  |
| 23 | cos(C, A) at L15 neutral p1 / p2 | 0.505 / 0.421 | `results/perposition_table_C_cosine.csv` | pod runner | not independently recomputed by me (Guiv) |  |  |
| 24 | cos(A, B) at L15 neutral p1 / p2 | −0.127 / −0.140 | `results/perposition_table_cosine.csv` | pod runner | not independently recomputed by me (Guiv) |  |  |
| 25 | ‖ΔW‖_F, A / C / D | 1.675 / 6.963 / 8.212 | `results/lora_delta_stats.json` | pod runner / tools/lora_delta_stats.py | not independently recomputed by me (Guiv) |  |  |
| 26 | Visibility V(neutral), A seed 0 / seed 1 | 0.1252 / 0.0919 (ratio 1.363) | `results/visibility_table.md` | pod runner / analysis/make_visibility_table.py | `tools/guiv_recompute.sh` row 26 → `notes/guiv_recompute_output.txt`; matches |  |  |
| 27 | Visibility V(neutral), D seed 0 / seed 1 | 0.3837 / 0.3910 (ratio 1.019) | `results/visibility_table.md` | pod runner | not independently recomputed by me (Guiv) |  |  |
| 28 | Visibility V(neutral), C | 0.5010 (highest of any arm) | `results/visibility_table.md` | pod runner | `tools/guiv_recompute.sh` row 28 → `notes/guiv_recompute_output.txt`; matches |  |  |
| 29 | Cross-seed cosine, D s0·s1, L15 neutral p1/p2 | 0.978 / 0.974 | `results/perposition_table_seeds_cosine.csv` | pod runner | not independently recomputed by me (Guiv) |  |  |
| 30 | Cross-seed cosine, A s0·s1, L15 neutral p1 at step 150 | 0.676 | `results/perposition_table_A_seeds_cosine.csv` | pod runner | `tools/guiv_recompute.sh` row 30 → `notes/guiv_recompute_output.txt`; matches |  |  |
| 31 | Steering, unsteered baseline | 26/200 = 0.130, EOS 0.140, mean len 470, numeral 0.130 | `results/steer_table.md, results/steer_eval/none_x1.json` | pod runner / tools/steer_eval.py | not independently recomputed by me (Guiv) |  |  |
| 32 | Steering, best cell D_math_full α=0.5 | 57/200 = 0.285, 44/13 discordant, p<1e-4 | `results/steer_table.md` | pod runner / tools/steer_table.py | not independently recomputed by me (Guiv) |  |  |
| 33 | Steering, random null at α=0.25 / 0.5 | mean 0.139 (0.110–0.170) / 0.134 (0.115–0.155), 5 seeds each | `results/steer_table.md` | pod runner | not independently recomputed by me (Guiv) |  |  |
| 34 | Judge calibration, gpt-5-mini / gemini-2.5-flash | 48/50 / 50/50 | `results/judge_calibration.jsonl` | pod runner / judge/calibrate.py | not independently recomputed by me (Guiv) |  |  |
| 35 | TF-IDF token-bag on real per-position lists | 8/150 correct; 125/150 predicted 'poetry'; null N1 3/20 | `results/lexical_on_lists.json` | pod runner / tools/lexical_on_lists.py | not independently recomputed by me (Guiv) |  |  |
| 36 | Module-family split of ΔW is uninformative | every arm within 0.02 of untrained N3 (MLP ~0.59 / lin-attn ~0.32 / full-attn ~0.09) | `results/lora_delta_family_split.json` | pod runner | not independently recomputed by me (Guiv) |  |  |
| 37 | Preflight: base completions hitting the 512 cap | 25/32 | `results/preflight_samples.json` | pod runner / tools/preflight.py | not independently recomputed by me (Guiv) |  |  |
| 38 | Identity check across the four code paths | passed; bos=None, add_special_tokens a no-op, eos=pad=248044 | `results/identity_check.json` | pod runner / tools/identity_check.py | not independently recomputed by me (Guiv) |  |  |
| 39 | Arm C corpus: kept / total, prompt coverage | 15,248/16,000 = 95.3%; 1,962/2,000 prompts | `data/C_samples.meta.json` | pod runner / grpo/train_sft.py sample | not independently recomputed by me (Guiv) |  |  |
| 40 | Pod cost | $200.81 over 14.38 h (+ $0.09 probe pod) | `CHANGELOG.md attempt ledger` | pod runner / RunPod API | not independently recomputed by me (Guiv) |  |  |
| 41 | SYNC DEFECT: results/acc_table.md was silently stale | was missing 50 lines (A, B, C, D_math, D_math_full and all paired counts); regenerated | `results/acc_table.md` | pod runner ship.sh rsync overwrote newer local file | not independently recomputed by me (Guiv) |  |  |
| 42 | SYNC DEFECT: results/visibility_table.md was silently stale | was missing the A seed-1 row; regenerated | `results/visibility_table.md` | same cause | not independently recomputed by me (Guiv) |  |  |
| 43 | SYNC DEFECT: results/lexical_items_perposition.jsonl was silently stale | had 66 rows instead of 102; rebuilt to 150 rows over all 7 arms | `results/lexical_items_perposition.jsonl` | same cause; tools/make_lexical_items.py | not independently recomputed by me (Guiv) |  |  |
| 44 | CORRECTION §7: 'cos to D at p0 rises 0.36 → 0.61' was wrong | neutral p0 is 0.357→0.335 (flat); math p0 is −0.253→0.611; the old line stitched two series | `results/emergence_A_early.csv` | pod runner; corrected in digest §7 | not independently recomputed by me (Guiv) |  |  |
| 45 | UNVERIFIABLE: arm B training curve | reward ≈0.07 / truncation 0.79 / mean length 456 — source logs/B_s0.log destroyed | `(none — not citable)` | pod runner; flagged in CLAIM_FIREWALL §2 | not independently recomputed by me (Guiv) |  |  |
...
| 51 | Guiv's reading — arm-D training corpus, 5 of 2,000 rows (244, 407, 575, 813, 1770; `random.Random(20260904)`) | all five cooking (recipes, techniques, substitutions); read as LLM-written; two of five are the same dish (matzo-ball soup, rows 407/813) → corpus topically narrower than its row count; no claim changes | `notes/READ_cooking_corpus.md`, `results/review_packet/cooking_samples.md` | Guiv (non-blind; ~6 min) | read | 5 |  |
| 52 | Guiv's reading — black-box generations, 5 prompts × 6 arm/seed rows, T 0.7, same sampling seed | divergence from base_s0 at token 1: D 5/5, D_math_full 1/5, A 0/5, B 0/5; cooking content in D 4/5; base_s1 vs base_s0 5/5 different (noise scale); base_s0 itself emits `<think>` on neutral_03 | `notes/READ_blackbox_rows.md`, `results/review_packet/blackbox_rows.md` | Guiv (non-blind; ~10 min) | read | 30 |  |
| 53 | **C_masked V(neutral p1) — the decision-line number** | 0.286 / 5.844 = 0.049 ≤ 0.18 → loss placement (line pre-stated in digest §12b / T2 §1.1) — one-liner output: `V = 0.0489 -> loss placement (<=0.18)` | `results/visibility_table_C_masked.md`, `results/lora_delta_stats_C_masked.json`, `results/perposition_table_C_masked.csv` | Chat 3 R1 / tools/visibility_C_masked.py | `tools/guiv_recompute.sh` row 53 → `notes/guiv_recompute_output.txt`; matches |  |  |
| 54 | C_masked trace, floor, constancy, L15 neutral p1 / p2 | 0.286 / 0.039 / 0.252 ; 0.257 / 0.042 / 0.189 (math p1 0.645 / 0.014 / 0.766) — one-liner output: `neutral p1 0.286 0.039 0.252 / neutral p2 0.257 0.042 0.189 / math p1 0.645 0.014 0.766 / math p2 0.651 0.086 0.31` | `results/perposition_table_C_masked.csv` | Chat 3 R1 / tools/per_position_diff.py | `tools/guiv_recompute.sh` row 54 → `notes/guiv_recompute_output.txt`; matches (first run crashed on the empty N1 floor cell; rerun output appended, matches) |  |  |
| 55 | C_masked held-out accuracy, both parsers | 187/200 = 0.935 raw and re-scored (no cut fires); vs A s0 4/5 p = 1.00; vs C s0 5/4 p = 1.00 — one-liner output: `raw 187 / 200 | re-scored 187 | cuts fired 0; acc_A_s0 C_masked-only 4 other-only 5 p=1.000; acc_C_s0 C_masked-only 5 other-only 4 p=1.000` (vs C s1: 7 / 5, p = 0.774) | `tools/guiv_recompute.sh` row 55 → `notes/guiv_recompute_output.txt`; matches | Chat 3 R1 / grpo/eval_acc.py |  |  |  |
| 56 | C_masked cosines, L15 neutral p1 / p2 | ·A s0 0.624 / 0.584; ·A s1 0.494 / 0.436; ·C s0 0.320 / 0.268; ·C s1 0.297 / 0.252 — one-liner output: `C_s0 p1 0.32, C_s1 p1 0.297, A_s0 p1 0.624, A_s1 p1 0.494, C_s0 p2 0.268, C_s1 p2 0.252, A_s0 p2 0.584, A_s1 p2 0.436` | `results/perposition_table_C_masked_cosine.csv` | Chat 3 R1 | `tools/guiv_recompute.sh` row 56 → `notes/guiv_recompute_output.txt`; matches |  |  |
| 57 | C_masked supervised-token fraction (completion + EOS over selected tokens) | 1,452,261 / 1,999,870 = 0.726 (547,609 prompt tokens masked; 8,792 selected rows) — one-liner output: `1452261 / 1999870 = 0.7262 | file says 0.7262 | rows 8792` | `results/supervised_fraction_C_masked.json` | Chat 3 R1 / tools/supervised_fraction_C_masked.py |  |  |  |
...
| 59 | C_masked ‖ΔW‖_F, max module, top σ | 5.844; 0.551 (layers.2.mlp.up_proj); 0.3550 — 84 % of C s0's 6.963 | `results/lora_delta_stats_C_masked.json` | Chat 3 R1 / tools/lora_delta_stats.py; merged 1d33df3 |  |  |  |
| 60 | C_masked : A trace ratio, L15 neutral p1 | 0.286/0.210 = 1.36, 0.286/0.155 = 1.85 (unmasked C: 16.63–22.63) | `results/perposition_table_C_masked_ratio.csv`, `results/perposition_table_C_masked.csv`, `results/trace_ratio_C_A_seeds.csv` | Chat 3 R1 |  |  |  |
| 61 | Failure-mode split of the 68 A-vs-D_math discordant items | D_math 42 FORMAT / 20 REASONING; A 0 FORMAT / 6 REASONING | `notes/guiv_tags.csv`, `results/llm_tags_68.csv` | Guiv (tags); LLM judge (independent pass) | read all 68 completion pairs against problem and gold; own labels (`notes/VERIFY_discordant_block.md`) | 68 |  |
| 62 | Judge–human concordance on failure mode | 67/68 identical; item 186 revised by Guiv (FORMAT → REASONING); item 56 omitted from his first pass and added afterwards | `notes/guiv_tags.csv` vs `results/llm_tags_68.csv` | both | item-by-item diff; **not a blind agreement rate** — tags produced after the judge's output was visible (see `notes/VERIFY_discordant_block.md` "Ordering and contamination") | 68 | 1 |
| 63 | X/Y → arm mapping is correct | all 6 reverse-discordant items confirmed | `results/discordant_key.json` (not opened for this check) | pipeline agent | arm identified from output-format signatures alone (A: `#### N` + `The answer is: N`; D_math: `<<…>>` / step-numbered markdown) — which also means the blinding leaks | 6 |  |
| 64 | FORMAT tags correspond to the re-parser's rescues | 40 of the 42 FORMAT items are D_math re-parser rescues; the 2 that are not (152, 184) are parser conservatism and survive into the 22 A-only-correct → 22/7 (p = 0.008) or 20/7 (p = 0.019) | `notes/guiv_tags.csv`, `results/acc_table_reparsed.md`, `results/reparse_rescued_ids.json` | Guiv (definition); Chat 1 (set intersection, digest §2) | set intersection of the two id lists | — | 2 |
| 65 | C_masked seed 1: V, trace, floor, ‖ΔW‖_F, accuracy, cos to seed 0 (R2) | V 0.047 = 0.277 / 5.844 (≤ 0.10 → confirms s0); floor 0.038; acc 189/200 both parsers; cos 0.735 (p1) | `results/visibility_table_R2.md`, `results/perposition_table_R2.csv`, `results/lora_delta_stats_C_masked_s1.json`, `results/acc_C_masked_s1.json`, `results/perposition_table_R2_cosine.csv` | replication chat R2 (`3ba31cb`) |  |  |  |
| 66 | C_scrambled: V and the prompt-loss confound (R2) | V 0.380 = 2.982 / 7.849 (≥ 0.30 large); train loss 1.736; prompt NLL base 7.011 → trained 5.383 on scrambled prompts vs 1.760 → 0.942 for C s1 on real prompts; cos to A s0 0.125, to C s0 0.425 | `results/visibility_table_R2.md`, `results/prompt_completion_loss_R2.json`, `results/perposition_table_R2_cosine.csv` | replication chat R2 |  |  |  |
| 67 | C_shifted: V (R2) | V 0.272 = 1.940 / 7.121 (0.18 < V < 0.30 inconclusive); cos to C s0 0.596, to A s0 0.325; acc 184/200 | `results/visibility_table_R2.md`, `results/perposition_table_R2_cosine.csv`, `results/acc_C_shifted_s0.json` | replication chat R2 |  |  |  |
| 68 | Ablation Δ, C s1 own direction α = 1 (E2) | 187 vs 185 at α = 0 → Δ +2 (≥ −3: not load-bearing); raw parser identical | `results/ablation_C_s1_own_a1.json`, `results/ablation_C_s1_none_a0.json`, `results/ablation_table.md` | E2 pod (`d675e8d`) / tools/ablate_trace.py |  |  |  |
| 69 | Ablation Δ, C_masked s0 own direction α = 1 (E2) | 188 vs 186 → Δ +2 (not load-bearing) | `results/ablation_C_masked_s0_own_a1.json`, `results/ablation_C_masked_s0_none_a0.json` | E2 pod |  |  |  |
| 70 | Ablation random matched-norm control, worst of 5 seeds, C s1 (E2) | worst Δ −2 (seed 3: 183) > −6 → control passes | `results/ablation_C_s1_rand{0..4}_a1.json` | E2 pod |  |  |  |
| 71 | Ablation random matched-norm control, worst of 5 seeds, C_masked s0 (E2) | worst Δ −1 (seed 1: 185) > −6 → control passes; base sanity d_C_s1 on base Δ −2 (153 vs 155; this session's base α = 0 is 3 / 5 below the saved 158 / 28) | `results/ablation_C_masked_s0_rand{0..4}_a1.json`, `results/ablation_base_*.json` | E2 pod |  |  |  |
| 72 | Null table: trace / N3 scaled to the arm's ‖ΔW‖_F, neutral p1 (C4 audit fork) | A s0 5.66, A s1 4.15, C_masked s0 2.21, B 2.57, C 22.6–22.7, D 17.3; N1 (unpaired) 0.747 exceeds A, B, C_masked, D_math traces | `results/null_table.md`, `tools/null_table.py` | audit fork (`e40c106`) |  |  |  |

## Readings by Guiv (2026-09-05; his notes verbatim in `notes/`)

- **Discordant items (`notes/VERIFY_discordant_block.md`, `notes/guiv_tags.csv`):** all 68 A-vs-D_math discordant pairs read against problem and gold, own FORMAT/REASONING labels; 67/68 concordant with the LLM judge, item 186 re-classified FORMAT → REASONING on the sharper definition (the gold value never appears as the conclusion of the chain), item 56 omitted from the first pass and added afterwards. **Ordering and contamination, in his words:** he had been given a characterisation of the original 20-item display file before tagging, so his calibration sample was redrawn from the remaining 48; his 68 tags were produced *after* the LLM judge's output existed and was visible, so 67/68 is verification of the judge's tags against raw text, not an independent agreement rate; the X/Y blinding leaks through output style (A: `#### N` + `The answer is: N`; D_math: `<<…>>` / step-numbered markdown), so the tagging was not arm-blind.
- **Cooking corpus (`notes/READ_cooking_corpus.md`):** 5 of 2,000 rows; all cooking; LLM-written register; two of five draws are the same dish → one limitation clause; no claim changes.
- **Black-box rows (`notes/READ_blackbox_rows.md`):** divergence from base_s0 at token 1 — D 5/5, D_math_full 1/5, A 0/5, B 0/5; base_s1 vs base_s0 differ on 5/5 (noise scale); `<think>` appears in the unsteered base_s0 on neutral_03.
- **Patchscope lists, blind** and **steered generations (8 prompts × 4 arms)**: done per PROGRAM_STATE; his written notes for these are not in the repo yet.
- **Recomputes (`tools/guiv_recompute.sh` → `notes/guiv_recompute_output.txt`):** rows 1, 2, 4, 12, 17, 22, 26, 28, 30, 49, 53, 54, 55, 56 — fourteen rows, all matching the digest (row 54's first run crashed on the empty N1 floor cell; the rerun output is appended and matches). Every other row in 1–45 is marked "not independently recomputed by me (Guiv)" in the table.
- **Not yet in the repo:** `notes/VERIFY_GUIV_columns.md` (his "raw items read" and "surprise-if-wrong" columns for rows 1–45, his own rows 46–64, and the two prose sections below) was not on `origin/guiv-notes` at merge time (`b38d076`); the two sections below stay empty until it lands.
