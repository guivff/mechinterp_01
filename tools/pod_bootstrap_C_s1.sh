#!/usr/bin/env bash
# C seed-1 replication: pod bootstrap on 2xH100 SXM, image runpod/pytorch:2.4.0-py3.11-cuda12.4.1.
...
uv pip freeze > logs/pod_packages_C_s1.txt
python - <<'PY'
import torch
...
print("imports ok")
PY
sha256sum data/C_samples.jsonl data/snippets/neutral.jsonl data/snippets/math.jsonl | tee logs/data_sha_C_s1.txt
grep -q '^78022b70295a1e0aec77d769b239263a8b8fe569ebded1d946642bcd2bbc109b  data/C_samples.jsonl' logs/data_sha_C_s1.txt
grep -q '^c8673772b35c0c9ebd42d183460aab30a5817d0436ea5cd845751eac9b0bd7a5  data/snippets/neutral.jsonl' logs/data_sha_C_s1.txt
grep -q '^483c37338e543d16af9b6e58dc3ca1e30d3081ba8b9e80d0a8c490d5c06c497c  data/snippets/math.jsonl' logs/data_sha_C_s1.txt
echo "data sha OK"
nvidia-smi --query-gpu=index,name,uuid,memory.total,driver_version --format=csv | tee logs/pod_gpu_inventory_C_s1.csv
...
python tools/identity_check.py --model "$MODEL" --model-revision "$MODEL_REVISION" --dataset-revision "$DATASET_REVISION" \
  --out results/identity_check_C_s1_pod.json 2>&1 | tee logs/identity_check_C_s1.log
python - <<'PY'
import json
a = json.load(open("results/identity_check.json")); b = json.load(open("results/identity_check_C_s1_pod.json"))
