...
def main() -> None:
    ...
    md = ["| arm | step | set | pos | raw ‖d‖ | constancy | base constancy | excess | base ‖h‖ |", "|---|---|---|---|---|---|---|---|---|"]
    fmt = lambda v: "—" if v is None else f"{v:.3f}"
    md += [f"| {r['arm']} | {r['step']} | {r['set']} | {r['position']} | {r['raw_norm']:.3f} | {fmt(r['constancy'])} | {r['base_constancy']:.3f} | {fmt(r['excess_constancy'])} | {r['base_norm']:.1f} |" for r in rows]
    md += ["", "| set | pos | x | y | cos |", "|---|---|---|---|---|"]
